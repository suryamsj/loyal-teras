...
<div class="flex-1 space-y-4 overflow-auto bg-gray-50 p-4 sm:space-y-6 md:p-6">
	<div class="flex w-full flex-col gap-4 sm:gap-6 md:flex-row md:items-center md:justify-between">
		...
		<div class="flex flex-col gap-3 sm:flex-row sm:space-x-3">
			<a
				href="/dashboard/smart-segmentation/automated-customer-segmentation"
				class="rounded-md bg-white p-3 text-center shadow-md transition-colors hover:bg-gray-50"
				>Automated Customer Segment</a
			>
			<a
				href="/dashboard/smart-segmentation/create-new-segment"
				class="bg-netral-dark hover:bg-opacity-90 rounded-md p-3 text-center text-white shadow-md transition-colors"
				>Create New Segment</a
			>
		</div>
	</div>
		...
		{#each realtimeCharts as item}
			<div class="rounded-lg bg-white p-4 shadow-sm md:p-6">
				<div class="flex justify-between">
					...
					<h3 class="mb-3 text-sm font-semibold text-gray-800 md:mb-4 md:text-xl">
						{item.value}
					</h3>
				</div>
				<div
				...
				<table class="w-full">
					<thead>
						<tr class="bg-gray-50 text-left">
							...
							<th class="px-6 py-3 text-sm font-medium text-gray-500">Type</th>
							<th class="px-6 py-3 text-sm font-medium text-gray-500">Total Members</th>
							<th class="px-6 py-3 text-sm font-medium text-gray-500">Last Updated</th>
					...
					<tbody class="divide-y divide-gray-100">
						{#each segments as segment}
							<tr class="hover:bg-gray-50">
								<td class="px-6 py-4 text-sm font-medium text-gray-700">{segment.name}</td>
								<td class="px-6 py-4 text-sm text-gray-500">{segment.type}</td>
								<td class="px-6 py-4 text-sm text-gray-500"
									>{segment.totalMembers.toLocaleString()}</td
								>
